...
public record VitalSignsDto(
        ...
        @Size(max = 6, message = "circunferência abdominal deve ter no máximo 6 caracteres")
        @Pattern(regexp = "^$|^\\d{1,3}([.,]\\d{1,2})?$", message = "circunferência abdominal deve ter formato numérico válido")
    ...
    public boolean isAbdominalCircumferenceWithinRange() {
        return isBlank(abdominalCircumferenceCm) || isDecimalInRange(abdominalCircumferenceCm, 0D, 300D);
    }
